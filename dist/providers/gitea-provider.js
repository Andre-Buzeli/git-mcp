...
class GiteaProvider extends base_provider_js_1.BaseVcsProvider {
    ...
    async listPackages(owner, repo, page = 1, limit = 30) {
        try {
            const data = await this.get(`/packages/${owner}`, {
                page,
                limit
            });
            return data.map((pkg) => ({
    ...
    async getPackage(owner, repo, packageId) {
        try {
            ...
            try {
                const versions = await this.get(`/packages/${owner}/${pkg.type}/${pkg.name}`);
                return {
                    ...pkg,
                    versions: versions
                };
    ...
    async deletePackage(owner, repo, packageId) {
        try {
            ...
            await this.delete(`/packages/${owner}/${pkg.type}/${pkg.name}/${pkg.version}`);
            return true;
        }
        catch (error) {
    ...
    async publishPackage(owner, repo, packageId) {
        try {
            ...
            await this.post(`/packages/${owner}/${pkg.type}/${pkg.name}/-/link/${repo}`);
            return true;
        }
        catch (error) {
    ...
    async downloadPackage(owner, repo, packageId) {
        try {
            ...
            const files = await this.get(`/packages/${owner}/${pkg.type}/${pkg.name}/${pkg.version}/files`);
    ...
    async listProjects(owner, repo, page = 1, limit = 30) {
        try {
            const data = await this.get(`/repos/${owner}/${repo}/projects`, {
                page,
                limit
            });
    ...
    async getProject(owner, repo, projectId) {
        try {
            const data = await this.get(`/repos/${owner}/${repo}/projects/${projectId}`);
            return {
                id: data.id.toString(),
                name: data.title,
                description: data.description,
    ...
    async createProject(owner, repo, projectData) {
        try {
            ...
            const data = await this.post(`/repos/${owner}/${repo}/projects`, payload);
            return {
                id: data.id.toString(),
                name: data.title,
                description: data.description,
    ...
    async updateProject(owner, repo, projectId, updates) {
        try {
            ...
            if (updates.state)
                payload.closed = updates.state === 'closed';
            const data = await this.patch(`/repos/${owner}/${repo}/projects/${projectId}`, payload);
            return {
                id: data.id.toString(),
                name: data.title,
                description: data.description,
    ...
    async deleteProject(owner, repo, projectId) {
        try {
            await this.delete(`/repos/${owner}/${repo}/projects/${projectId}`);
            return true;
        }
        catch (error) {
    ...
    async listProjectItems(owner, repo, projectId, page = 1, limit = 30) {
        try {
            ...
            try {
                const data = await this.get(`/repos/${owner}/${repo}/projects/${projectId}/columns`);
                const allCards = [];
                for (const column of data) {
                    try {
                        const cards = await this.get(`/repos/${owner}/${repo}/projects/columns/${column.id}/cards`);
