...
const ReleasesInputSchema = z.object({
  ...
  // Para update
  new_tag_name: z.string().optional(),
  new_name: z.string().optional(),
  new_body: z.string().optional(),
  new_draft: z.boolean().optional(),
  new_prerelease: z.boolean().optional(),
  new_target_commitish: z.string().optional(),
 ...
 */
const ReleasesResultSchema = z.object({
  success: z.boolean(),
  action: z.string(),
  message: z.string(),
  data: z.any().optional(),
...
export const releasesTool = {
  ...
  async updateRelease(params: ReleasesInput, provider: VcsOperations): Promise<ReleasesResult> {
    try {
      ...
      if (params.new_tag_name) updateData.tag_name = params.new_tag_name;
      if (params.new_name) updateData.name = params.new_name;
      if (params.new_body !== undefined) updateData.body = params.new_body;
      if (params.new_draft !== undefined) updateData.draft = params.new_draft;
      if (params.new_prerelease !== undefined) updateData.prerelease = params.new_prerelease;
      if (params.new_target_commitish) updateData.target_commitish = params.new_target_commitish;
