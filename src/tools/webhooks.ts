 ...
 */
const WebhooksInputSchema = z.object({
  ...
  new_content_type: z.enum(['json', 'form']).optional(),
  new_secret: z.string().optional(),
  new_events: z.array(z.string()).optional(),
  new_active: z.boolean().optional(),
});
 ...
 */
const WebhooksResultSchema = z.object({
  success: z.boolean(),
  action: z.string(),
  message: z.string(),
  data: z.any().optional(),
...
export const webhooksTool = {
  ...
  async updateWebhook(params: WebhooksInput, provider: VcsOperations): Promise<WebhooksResult> {
    try {
      ...
      if (params.new_url) updateData.url = params.new_url;
      if (params.new_content_type) updateData.content_type = params.new_content_type;
      if (params.new_secret) updateData.secret = params.new_secret;
      if (params.new_events) updateData.events = params.new_events;
      if (params.new_active !== undefined) updateData.active = params.new_active;
